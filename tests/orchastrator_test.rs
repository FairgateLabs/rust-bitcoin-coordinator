use bitcoin::{Amount, Network, ScriptBuf, TxOut, Txid};
...
fn monitor_instance_test() -> Result<(), anyhow::Error> {
    ...
    let tx_id =
        Txid::from_str("8904aba41b91cc59eea5f5767bf8fbd5f8d861629885267379cae615c8115be1").unwrap();
...
fn speed_up_tx() -> Result<(), anyhow::Error> {
    ...
    // Make a orchastrator tick and monitor is ready
    // Orchastrator should go out because there is no new block.
    // Make a new tick and monitor is ready
    // Status for tx monitor should say that tx_was_seen is false.
    // Mock dispatcher speed up.
